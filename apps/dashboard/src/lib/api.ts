...
export async function executeWorkflow(workflow: WorkflowDefinition, taskId?: string): Promise<{ taskId: string }> {
...
export async function getTaskStatus(taskId: string): Promise<{ taskId: string; state: string; message?: string }> {
  const res = await fetch(`${BASE}/tasks/${taskId}`);
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}

export async function listRunningTasks(): Promise<{ tasks: Array<{ taskId: string; state: string; message?: string }> }> {
