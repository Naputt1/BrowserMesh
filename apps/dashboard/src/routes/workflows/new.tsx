...
function NewWorkflowPage() {
  ...
  const events = useTaskStore((s) => taskId ? (s.eventsByTaskId[taskId] ?? EMPTY_EVENTS) : EMPTY_EVENTS);
  useTaskEvents(taskId);

  const lastEvent = events[events.length - 1];
  const taskStatus: string = lastEvent?.type === "task_completed" ? "completed"
          ...
          <div className="border-t bg-white shrink-0 max-h-[50vh] overflow-y-auto">
            <div className="px-4 py-3 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground font-mono">{taskId.slice(0, 16)}...</span>
              </div>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="space-y-1 text-xs">
                        {partialData.map((e, i) => (
                          <div key={i} className="flex items-start gap-2 py-0.5 border-b border-gray-100 last:border-0">
                            <span className="font-medium text-purple-700 shrink-0 min-w-[60px]">{e.path}</span>
