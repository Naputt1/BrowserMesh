...
import type { WorkflowDefinition, WorkflowNode, WorkflowEdge, NodeType, WorkflowEvent, GlobalSettings } from "@browsermesh/workflow";
import { WorkflowInterpreter, type InterpreterOptions } from "../interpreter/workflow-interpreter.js";
import type { Page, Locator, CustomHandler, ExecutionContext, NodeHandler } from "../interpreter/types.js";
...
describe("WorkflowInterpreter — fetch", () => {
  it("executes a basic GET request via page.evaluate", async () => {
    const page = mockPage();
    vi.mocked(page.evaluate).mockResolvedValue(JSON.stringify({
      status: 200,
      statusText: "OK",
    ...
    const intr = new WorkflowInterpreter(makeOpts({
      ...
      workflow: wf({
        ...
        nodes: [
          node("s", "start"),
          node("f", "fetch", { method: "GET", url: "https://api.example.com/data", actLikeNavigation: false }),
          node("e", "end"),
        ],
        edges: [
          edge("e1", "s", "flow", "f", "flow"),
          edge("e2", "f", "flow", "e", "flow"),
        ],
      }),
    }));
  ...
  it("substitutes variables from data inputs", async () => {
    const page = mockPage();
    vi.mocked(page.evaluate).mockResolvedValue(JSON.stringify({ status: 200, statusText: "OK", headers: {}, body: "" }));
    const intr = new WorkflowInterpreter(makeOpts({
      page,
      workflow: wf({
        settings: { multiPage: false },
      ...
      workflow: wf({
        nodes: [
          ...
          node("f", "fetch", {
            ...
            headers: [{ key: "Authorization", value: "Bearer token123" }, { key: "X-Custom", value: "test" }],
            actLikeNavigation: false,
          }),
          node("e", "end"),
        ],
  ...
  it("injects interception script via addInitScript", async () => {
    ...
    const intr = new WorkflowInterpreter(makeOpts({
      ...
      workflow: wf({
        ...
        edges: [
          edge("e1", "s", "flow", "l", "flow"),
          edge("e2", "l", "flow", "e", "flow"),
        ],
      }),
    }));
    await collect(intr.execute());
...
describe("WorkflowInterpreter — state", () => {
  it("set then get a value in the same workflow", async () => {
    ...
    const intr = new WorkflowInterpreter({ workflow: wfDef, page, taskId: "t1", stateStore: store });
    const events = await collect(intr.execute());
    expect(events.filter((e) => e.type === "task_failed")).toHaveLength(0);
  ...
  it("increment operation adds to existing value", async () => {
    ...
    const wfDef = wf({
      ...
      edges: [
        edge("e1", "s", "flow", "st", "flow"),
        edge("e2", "st", "flow", "e", "flow"),
      ],
    });
...
describe("WorkflowInterpreter — page", () => {
  ...
  it("create action creates a new page", async () => {
    ...
    const pageManager = {
      ...
      listPages: vi.fn().mockReturnValue([{ pageId: "default", url: "about:blank" }, { pageId: "tab2", url: "about:blank" }]),
...
describe("WorkflowInterpreter — pageKey routing via data pins", () => {
  ...
  it("fetch with pageKey routes to that page", async () => {
    const page = mockPage();
    const pm = makePageManager(page);
    vi.mocked(page.evaluate).mockResolvedValue({ status: 200, statusText: "OK", headers: {}, body: "" });
    const intr = new WorkflowInterpreter(makeOpts({
      page,
      workflow: wf({
        nodes: [
          node("s", "start"),
          node("p", "page", { action: "create", pageId: "t1" }),
          node("f", "fetch", { method: "GET", url: "https://example.com/api", actLikeNavigation: false }),
          node("e", "end"),
        ],
        edges: [
          edge("e1", "s", "flow", "p", "flow"),
  ...
  it("navigate without pageKey uses context.page directly", async () => {
    ...
    const intr = new WorkflowInterpreter(makeOpts({
      ...
      workflow: wf({
        ...
        edges: [
          edge("e1", "s", "flow", "n", "flow"),
          edge("e2", "n", "flow", "e", "flow"),
        ],
      }),
      pageManager: pm,
    }));
...
describe("fetchHandler — direct", () => {
  it("throws when url is missing", async () => {
    ...
    await expect(async () => { for await (const _ of gen) {} }).rejects.toThrow("fetch node requires a url in config");
  });

  it("outputs response via setOutput", async () => {
    const page = mockPage();
    const mockResponse = { status: 200, statusText: "OK", headers: { "content-type": "text/plain" }, body: "hello" };
    vi.mocked(page.evaluate).mockResolvedValue(mockResponse);
    const setOutput = vi.fn();

    const node = { id: "f1", type: "fetch" as NodeType, config: { method: "GET", url: "https://example.com", actLikeNavigation: false } };
    const context = { page, taskId: "t1", signal: new AbortController().signal, getCustomHandler: () => undefined, setOutput } as unknown as ExecutionContext;
...
describe("listenHandler — URL pattern matching", () => {
  it("waits for requests and reads them from page", async () => {
    ...
    const node = { id: "l1", type: "listen" as NodeType, config: { urlPatterns: ["/api/*"], waitMs: 0 } };
    const context = { page, taskId: "t1", signal: new AbortController().signal, getCustomHandler: () => undefined, setOutput } as unknown as ExecutionContext;
