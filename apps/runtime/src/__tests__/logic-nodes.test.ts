...
describe("compareHandler — direct", () => {
  function makeContext(setOutput = vi.fn(), overrides?: Partial<ExecutionContext>): ExecutionContext {
    return {
      taskId: "t1",
  ...
  async function runHandler(node: WorkflowNode, context: ExecutionContext, inputs: Record<string, unknown>) {
...
describe("andHandler — direct", () => {
  async function run(a: unknown, b: unknown) {
    const setOutput = vi.fn();
    const context = { taskId: "t1", signal: new AbortController().signal, page: mockPage(), getCustomHandler: () => undefined, setOutput } as unknown as ExecutionContext;
...
describe("ifHandler — direct", () => {
  it("executes true subgraph when condition is truthy", async () => {
    ...
    const context = { taskId: "t1", signal: new AbortController().signal, page: mockPage(), getCustomHandler: () => undefined, setOutput: vi.fn() } as unknown as ExecutionContext;
  ...
  it("does nothing when executeSubgraph is undefined", async () => {
    const gen = ifHandler(
      ...
      { taskId: "t1", signal: new AbortController().signal, page: mockPage(), getCustomHandler: () => undefined, setOutput: vi.fn() } as unknown as ExecutionContext,
      { condition: true },
      undefined,
    );
    for await (const _ of gen) {}
...
describe("switchHandler — direct", () => {
  it("executes matching case subgraph", async () => {
    const executeSubgraph = vi.fn().mockReturnValue(async function* () {}());
    const context = { taskId: "t1", signal: new AbortController().signal, page: mockPage(), getCustomHandler: () => undefined, setOutput: vi.fn() } as unknown as ExecutionContext;
    const node = { id: "sw1", type: "switch" as NodeType, config: { cases: [{ label: "Admin", value: "admin" }, { label: "User", value: "user" }] } };
  ...
  it("executes default subgraph when no case matches", async () => {
    const executeSubgraph = vi.fn().mockReturnValue(async function* () {}());
    const context = { taskId: "t1", signal: new AbortController().signal, page: mockPage(), getCustomHandler: () => undefined, setOutput: vi.fn() } as unknown as ExecutionContext;
    const node = { id: "sw1", type: "switch" as NodeType, config: { cases: [{ label: "Admin", value: "admin" }] } };
  ...
  it("matches case by string value", async () => {
    const executeSubgraph = vi.fn().mockReturnValue(async function* () {}());
    const context = { taskId: "t1", signal: new AbortController().signal, page: mockPage(), getCustomHandler: () => undefined, setOutput: vi.fn() } as unknown as ExecutionContext;
    const node = { id: "sw1", type: "switch" as NodeType, config: { cases: [{ label: "Num", value: "42" }] } };
...
describe("breakHandler — direct", () => {
  it("sets controlSignal.value to 'break'", async () => {
    ...
    const context = { taskId: "t1", signal: new AbortController().signal, page: mockPage(), getCustomHandler: () => undefined, setOutput: vi.fn(), controlSignal } as unknown as ExecutionContext;
    ...
    const intr = new WorkflowInterpreter(makeOpts({
      ...
      workflow: wf({
        nodes: [
          ...
          node("sw", "switch", { cases: [{ label: "Admin", value: "admin" }, { label: "User", value: "user" }] }),
