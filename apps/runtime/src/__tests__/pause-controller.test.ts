...
describe("PauseController", () => {
  ...
  it("waitIfPaused blocks while paused and resolves on resume", async () => {
    ...
    let resolved = false;
    const waitPromise = pc.waitIfPaused().then(() => { resolved = true; });
  ...
  it("multiple waiters all resolve on resume", async () => {
    ...
    let count = 0;
    const w1 = pc.waitIfPaused().then(() => { count++; });
    const w2 = pc.waitIfPaused().then(() => { count++; });
    const w3 = pc.waitIfPaused().then(() => { count++; });
