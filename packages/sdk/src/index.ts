...
export class BrowserMeshClient {
  private readonly client: {
    ...
    ListRunningTasks(request: object, callback: (err: Error | null, response: object) => void): void;
  };
  ...
  constructor(readonly options: BrowserMeshClientOptions) {
    if (options._grpcClient) {
      ...
      const Ctor = pkg?.BrowserMeshRuntime as new (endpoint: string, creds: grpc.ChannelCredentials) => typeof this.client;
    ...
    return new Promise((resolve, reject) => {
      this.client.ListRunningTasks({}, (err: Error | null, response: object) => {
        if (err) {
          ...
          const raw = response as { tasks?: Array<{ task_id: string; state: string; message?: string }> };
          resolve(
            (raw.tasks ?? []).map((t) => ({
              taskId: t.task_id,
              state: t.state,
  ...
  private unaryCall(method: "CancelTask" | "PauseTask" | "ResumeTask" | "GetTaskStatus", request: object): Promise<TaskStatusResult> {
    return new Promise((resolve, reject) => {
      (this.client[method] as (req: object, cb: (err: Error | null, response: object) => void) => void)(request, (err: Error | null, response: object) => {
        if (err) {
          reject(err);
        } else {
  ...
  private decodeWorkflowEvent(raw: Record<string, unknown>): WorkflowEvent {
    ...
    if (raw.step_started) {
      ...
      return { ...base, type: "step_started", stepId: ev.step_id, stepType: ev.step_type as WorkflowNode["type"] };
    }
    if (raw.step_completed) {
      const ev = raw.step_completed as { step_id: string; output_json?: string };
      return { ...base, type: "step_completed", stepId: ev.step_id, output: ev.output_json ? JSON.parse(ev.output_json) : undefined };
    }
    if (raw.partial_data) {
    ...
    if (raw.log) {
      ...
      return { ...base, type: "log", level: ev.level as "debug" | "info" | "warn" | "error", message: ev.message };
    }
    if (raw.screenshot) {
      const ev = raw.screenshot as { label: string; data: Uint8Array; mime_type: string };
      return { ...base, type: "screenshot", label: ev.label, data: ev.data, mimeType: ev.mime_type as "image/png" | "image/jpeg" | "image/webp" };
    }
    if (raw.progress) {
      const ev = raw.progress as { completed_steps: number; total_steps: number; message?: string };
      return { ...base, type: "progress", completedSteps: ev.completed_steps, totalSteps: ev.total_steps, message: ev.message ?? undefined };
    }
    if (raw.task_completed) {
      const ev = raw.task_completed as { result_json?: string };
      return { ...base, type: "task_completed", result: ev.result_json ? JSON.parse(ev.result_json) : undefined };
    }
    if (raw.task_failed) {
      const ev = raw.task_failed as { error_code: string; message: string; retryable: boolean };
      return { ...base, type: "task_failed", errorCode: ev.error_code, message: ev.message, retryable: ev.retryable };
    }
