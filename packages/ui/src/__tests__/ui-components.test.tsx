...
describe('TaskConsole', () => {
  ...
  it('renders progress event', () => {
    const ts = new Date().toISOString();
    const events = [
      { type: 'progress' as const, taskId: 't1', timestamp: ts, completedSteps: 3, totalSteps: 10 },
    ];
    render(<TaskConsole events={events} />);
  ...
  it('renders partial_data event', () => {
    const ts = new Date().toISOString();
    const events = [
      { type: 'partial_data' as const, taskId: 't1', timestamp: ts, path: 'title', value: 'Hello' },
    ];
    render(<TaskConsole events={events} />);
...
describe('ContextMenu', () => {
  ...
  it('calls item onClick when clicked', () => {
    const onClick = vi.fn();
    render(<ContextMenu x={0} y={0} items={[{ label: 'Edit', onClick }]} onClose={() => {}} />);
  ...
  it('closes on Escape key', () => {
    const onClose = vi.fn();
    render(<ContextMenu x={0} y={0} items={[{ label: 'Item' }]} onClose={onClose} />);
  ...
  it('renders items with color dot', () => {
    render(
      <ContextMenu
        x={0}
        y={0}
        items={[{ label: 'Colored', color: '#ff0000' }]}
        onClose={() => {}}
      />,
    );
  ...
  it('renders items with icon', () => {
    render(
      <ContextMenu
        x={0}
        y={0}
        items={[{ label: 'With Icon', icon: <span data-testid="icon">🔍</span> }]}
        onClose={() => {}}
      />,
    );
...
describe('ExtractionMapper', () => {
  it('removes a field when clicking remove', () => {
    ...
    const removeBtn = Array.from(removeButtons).find((b) => b.querySelector('svg'));
