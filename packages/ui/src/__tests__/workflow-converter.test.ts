...
import type { WorkflowDefinition, WorkflowNode, WorkflowEdge, DataType } from "@browsermesh/workflow";
...
describe("reactFlowToWorkflow", () => {
  it("converts React Flow nodes back to WorkflowNodes", () => {
    const rfNodes = [
      { id: "n1", type: "workflowNode", position: { x: 0, y: 0 }, data: { label: "Start", nodeType: "start" as const, config: {} } },
      { id: "n2", type: "workflowNode", position: { x: 200, y: 0 }, data: { label: "End", nodeType: "end" as const, config: {} } },
    ];
    const rfEdges = [{ id: "e1", source: "n1", sourceHandle: "flow", target: "n2", targetHandle: "flow" }];
