...
import { DebugStateStore } from '../state-store';

const stateStores = new Map<string, DebugStateStore>();

export const stateHandler: DebugNodeHandler = async function* (node, context, inputs) {
  ...
  let store = stateStores.get(context.taskId);
  if (!store) {
    store = new DebugStateStore();
    stateStores.set(context.taskId, store);
  }
