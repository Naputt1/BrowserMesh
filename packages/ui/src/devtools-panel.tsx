...
export function DevtoolsPanel({ logs }: DevtoolsPanelProps) {
  ...
  const logEvents = logs?.filter((e): e is WorkflowEvent & { type: "log" } => e.type === "log") ?? [];
      ...
      <div className="flex-1 overflow-auto p-2 font-mono text-xs space-y-1">
        {tab === "console" && (
          logEvents.length === 0 ? (
            ...
            logEvents.map((e, i) => (
              <div key={i} className={`${e.level === "error" ? "text-red-400" : e.level === "warn" ? "text-yellow-400" : e.level === "info" ? "text-blue-400" : "text-gray-300"}`}>
        ...
        {tab === "network" && (
          <div className="text-gray-500 italic">Network log coming soon</div>
        )}
      </div>
    </div>
  );
