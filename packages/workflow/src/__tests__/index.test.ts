...
const ALL_NODE_TYPES = [
  ...
  "state", "page", "if", "switch", "and", "or", "not",
  "break", "compare", "continue",
] as const;
...
describe("NODE_DEFINITIONS", () => {
  ...
  it("action nodes have at least flow output", () => {
    const actionNodes = ["navigate", "click", "type", "wait", "scroll", "custom", "fetch", "listen", "page"];
  ...
  it("compare outputs boolean result", () => {
    ...
    expect(def.outputs.some((p) => p.name === "result" && p.dataType?.kind === "boolean")).toBe(true);
  });
